...
# Redis setup for caching (Vercel-compatible with Redis add-ons like Upstash)
try:
    redis_client = redis.Redis.from_url(os.getenv("REDIS_URL", "redis://localhost:6379"), decode_responses=True)
    redis_client.ping()
except redis.RedisError:
    logging.warning("Redis unavailable, falling back to in-memory caching")
    redis_client = None
...
class LLMClient:
    ...
    async def query_async(self, messages, max_tokens=300, temperature=0.5):
        cache_key = f"llm:{json.dumps(messages)}:{max_tokens}:{temperature}"
        if redis_client:
        ...
        async with aiohttp.ClientSession() as session:
            for attempt in range(3):
                try:
                    ...
                    if e.status in (429, 401, 402, 404):
                        if e.status == 401:
...
executor = ThreadPoolExecutor(max_workers=2)  # For async image processing
...
async def process_image_async(file):
    ...
    try:
        img = await loop.run_in_executor(executor, Image.open, file)
        await loop.run_in_executor(executor, img.verify)
        img = await loop.run_in_executor(executor, Image.open, file)
        img = img.convert('RGB')
        img = img.resize((300, 225), Image.Resampling.LANCZOS)  # Further reduced size
        buffer = io.BytesIO()
        img.save(buffer, format='JPEG', quality=80)  # Lower quality
        return base64.b64encode(buffer.getvalue()).decode("utf-8")
    except Exception as e:
        logging.error(f"Image processing failed: {str(e)}")
        return None

async def translate_text_async(text, source_lang, target_lang):
    if source_lang == target_lang or target_lang == "en":
        return text
    cache_key = f"translate:{text}:{source_lang}:{target_lang}"
    if redis_client:
        cached = redis_client.get(cache_key)
        if cached:
    ...
    data = await llm_client.query_async(messages, max_tokens=1000, temperature=0.3)
    if 'error' in data:
        logging.error(f"Translation failed: {data['error']}")
        return text
    if redis_client:
        redis_client.setex(cache_key, 3600, data['content'])  # Cache for 1 hour
    return data['content']

@app.errorhandler(Exception)
def handle_error(error):
    logging.error(f"Server error: {str(error)}", exc_info=True)
    return jsonify({"error": "Internal server error. Please try again later.", "typing_effect": True}), 500
...
async def set_prefs():
    try:
        ...
        return jsonify({"error": f"Error setting preferences: {str(e)}", "typing_effect": True}), 500
...
async def chat():
    try:
        ...
        if not query:
            ...
            return jsonify({"error": await translate_text_async(error_msg, "en", user_prefs['language']), "typing_effect": True}), 400
        
        query_en = await translate_text_async(query, user_prefs['language'], "en") if user_prefs['language'] != "en" else query
        ...
        data = await llm_client.query_async(messages, max_tokens=1000, temperature=0.5)
...
- Contact local agricultural extension services in {user_prefs['region'] or 'India'} for assistance.
                """
                return jsonify({
                    "response": format_text_for_display(await translate_text_async(fallback_response, "en", user_prefs['language'])),
                    "model": "none",
                    "typing_effect": True
                })
            return jsonify({"error": await translate_text_async(error_msg, "en", user_prefs['language']), "typing_effect": True}), 429
        
        response_text = await translate_text_async(data['content'], "en", user_prefs['language'])
        ...
        return jsonify({"error": f"Chat error: {str(e)}", "typing_effect": True}), 500

@lru_cache(maxsize=100)
async def get_region_specific_remedy(plant_name, scientific_name, disease_name, region, language, plant_id_remedy, disease_description):
    cache_key = f"remedy:{plant_name}:{scientific_name}:{disease_name}:{region}:{language}"
    if redis_client:
        cached = redis_client.get(cache_key)
        if cached:
...
For {region or 'India'}, consult local agricultural extension services for region-specific advice due to API limitations.
        """
        translated = await translate_text_async(fallback, "en", language)
        if redis_client:
            redis_client.setex(cache_key, 3600, translated)
        return translated, 'none'
    translated = await translate_text_async(data['content'], "en", language)
    if redis_client:
        redis_client.setex(cache_key, 3600, translated)
    return translated, data['model']

@lru_cache(maxsize=100)
async def enhance_identification(plant_name, scientific_name, plant_confidence, disease_name, disease_confidence, region, language, disease_description):
    cache_key = f"id:{plant_name}:{scientific_name}:{disease_name}:{region}:{language}"
    if redis_client and redis_client.get(cache_key):
        logging.info("Returning cached identification")
        return tuple(json.loads(redis_client.get(cache_key)))
    ...
    if plant_confidence < 0.3 or disease_name in ['Fungi', 'Abiotic']:
        ...
        data = await llm_client.query_async(messages, max_tokens=500, temperature=0.3)
        if 'error' in data or 'content' not in data:
            result = (plant_name, scientific_name, plant_confidence, disease_name, disease_confidence)
            if redis_client:
...
@lru_cache(maxsize=100)
async def perenual_validation(plant_name, region):
    cache_key = f"perenual:{plant_name}:{region}"
    if redis_client and redis_client.get(cache_key):
    ...
    try:
        ...
        for file in files:
            if not file.filename or not allowed_file(file.filename):
                ...
                return jsonify({"error": await translate_text_async(error_msg, "en", user_prefs['language']), "typing_effect": True}), 400
            encoded = await process_image_async(file)
            if not encoded:
        ...
        async with aiohttp.ClientSession() as session:
            plant_id_response = await session.post(
            ...
            plant_data = await plant_id_response.json()
            ...
            if plant_confidence < 0.3 and PERENUAL_API_KEY:
                plant_name, scientific_name, perenual_confidence = await perenual_validation(plant_name, user_prefs['region'])
                plant_confidence = max(plant_confidence, perenual_confidence)

            disease_resp = await session.post(
            ...
            disease_data = await disease_resp.json()
...
- Monitor plant response after treatment
        ...
        if cache_key_id in identification_cache:
            ...
            plant_name, scientific_name, plant_confidence, disease_name, disease_confidence = await enhance_identification(
        ...
        if cache_key_remedy in remedy_cache:
            ...
            enhanced_remedy, selected_model = await get_region_specific_remedy(
        ...
        return jsonify({
            "plant": await translate_text_async(plant_name, "en", user_prefs['language']),
            "scientific_name": await translate_text_async(scientific_name, "en", user_prefs['language']),
            "plant_confidence": f"{plant_confidence:.2%}",
            "disease": await translate_text_async(disease_name, "en", user_prefs['language']),
        ...
        return jsonify({"error": await translate_text_async(f"Failed to process image or connect to API: {str(e)}", "en", user_prefs['language']), "typing_effect": True}), 500
